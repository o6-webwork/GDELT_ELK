...
if active_task_id and st.session_state.get('polling_active'):
    ...
    cancel_button_placeholder = st.empty() # Define the placeholder once
    ...
    while st.session_state['polling_active']:
        ...
        if task_data:
            ...
            # Update UI elements in the status placeholder
            ...
            # Conditionally display the cancel button IN the placeholder
            if status in ["running", "starting", "aborting"]:
                 # Use the placeholder to create the button IF NEEDED
                 # The key ensures it's the same logical button Streamlit tracks
                 if cancel_button_placeholder.button("Cancel Task", key=f"cancel_{active_task_id}"):
                     ...
                     if cancel_result:
                         ...
                         # Don't clear the button immediately, let the status update show 'aborting'
                     else:
                         st.error("Failed to send cancellation request.")
                         cancel_button_placeholder.empty() # Clear button on failure to cancel
            else:
                 # Clear the placeholder if the button shouldn't be visible
                 cancel_button_placeholder.empty()

            # Stop polling if task is finished, aborted, or errored
            if status in ["completed", "aborted", "error"]:
                st.session_state['polling_active'] = False
                # Display final message outside the polling loop section below
                # We break here, and the rerun will show the final status block
                status_placeholder.empty() # Clear dynamic status area
                progress_bar.empty() # Clear progress bar
                cancel_button_placeholder.empty() # Ensure button is gone
                st.rerun() # Rerun to display the final status block cleanly
                # break # Exit polling loop (st.rerun() stops execution anyway)

        else:
            # Task not found or error fetching status
            st.session_state['polling_active'] = False
            st.error(f"Could not retrieve status for task {active_task_id}. Stopping status updates.")
            status_placeholder.empty() # Clear dynamic status area
            progress_bar.empty() # Clear progress bar
            cancel_button_placeholder.empty() # Ensure button is gone
            st.rerun() # Rerun to show the error and potentially the last known status block
            # break # Exit polling loop (st.rerun() stops execution anyway)

        # Wait before polling again
        time.sleep(POLL_INTERVAL_SECONDS)

# Display the final status when polling stops or if there was a previous task run
elif st.session_state.get('task_status_data'):
    # Display the last known status if polling is not active but data exists
