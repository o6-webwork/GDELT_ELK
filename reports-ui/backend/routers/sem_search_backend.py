...
def sem_search(req: SearchRequest):
    ...
    # Get data from ES
    respone = load_json_from_elasticsearch(es_query)

    # Filter data from ES
    hits = respone.get("hits", {}).get("hits", [])
    ...
    filtered_data = {
        "took": respone.get("took"),
        "timed_out": respone.get("timed_out"),
        "_shards": respone.get("_shards"),
        "hits": {
            "total": {
    ...
    articles = []
    for hit in filtered_data["hits"]["hits"]:
        source = hit["_source"]
        articles.append({
